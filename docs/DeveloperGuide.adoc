...
After forking the repo, the documentation will still have the SE-EDU branding and refer to the `se-edu/cardcollection-level4` repo.

If you plan to develop this fork as a separate product (i.e. instead of contributing to `se-edu/cardcollection-level4`), you should do the following:
...
[NOTE]
Coverage reporting could be useful for a team repository that hosts the final version but it is not that useful for your flashcardal fork.
...
Classes used by multiple components are in the `seedu.cardcollection.commons` package.
...
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/address/model/CardCollection.java[`CardCollection`] and link:{repoURL}/src/main/java/seedu/address/model/flashcard/Person.java[`Person`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/address/model/CardCollection.java[`CardCollection`] allows you to update a flashcard, and link:{repoURL}/src/main/java/seedu/address/model/flashcard/Person.java[`Person`] allows you to update the tags.
* Solution
