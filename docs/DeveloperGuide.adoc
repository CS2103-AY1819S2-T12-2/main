...
`Statistics` acts as a container to store the data for calculating success percentage from quiz mode. It stores `successAttempt` and `attemptNumber`.
...
* `Statistics#merge(Statistics other)` -- Return merged statistics of two given statistics, generally to be used to calculate the success rate from a bunch of flashcards with a particular tag.

When a user queries a statistics by using command `stats`, `StatsCommand` is the one to calculate the statistics. Currently, `StatsCommand` only supports an overall success rate from all user's flashcards. It will asks `Model` to return all of the flashcards and then merge each `Statistics` from each flashcard to return an overall cummulative `Statistics`. 
...
** Cons: User has less flexibility in term of querying the statistics (User can not know a certain flashcard statistics)
