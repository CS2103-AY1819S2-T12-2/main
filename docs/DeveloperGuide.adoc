...
It extends `CardCollection` with an undo/redo history, stored internally as a `cardCollectionStateList`, `commandHistory` and `currentStatePointer`.
...
//tag::srs[]
=== Spaced Repetition System (SRS) Feature
...
SRS is facilitated by `Proficiency` class. In this section, you will read the API from `Proficiency` class and the current implementation of SRS feature.

==== Proficiency Class API
...
image::QuizStartSRSSequenceDiagram.png[width="800"]

image::QuizEndSRSSequenceDiagram.png[width="800"]
...
image::StatsCommandSequenceDiagram.png[width="800"]
