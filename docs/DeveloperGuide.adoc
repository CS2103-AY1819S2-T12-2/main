...
`Statistics` acts as a container to store the data for calculating success percentage from quiz mode. +
...
When a user queries a statistics by using command `stats`, `StatsCommand` is the one to calculate the statistics. +
Currently, `StatsCommand` only supports an overall success rate from all user's flashcards. It will asks `Model` to +
