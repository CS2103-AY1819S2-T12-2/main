...
The SRS mode only chooses flashcards that is already time to show to the user.
...
The `ShowCommand` is handled similarly with the `QuizCommand`. Instead of `MainWindow`, `CardViewPanel` listens to the change in quiz state mode inside `Model`. The `CardViewPanel` creates a new `FlashcardCardView` with the correct state (with back face hidden accordingly). Then, the displayed flashcard will be replaced. Below is the sequence diagram of how the show operation works:
...
** Cons: Need to be able to swap the `MainWindow`'s components when the quiz starts in order to replace the card collection list view with the quiz panel.
* **Alternative 2:** Use a separate window for quiz mode.
** Pros: Quiz logic separate from `MainWindow`. Easily customizable and more flexible. Much easier to handle GUI,
since we only need to spawn a new window and close it afterwards.
** Cons: Requires a separate command box for the quiz mode window.
Also may need to be able to hide the main window so that users cannot see the card collection list.
